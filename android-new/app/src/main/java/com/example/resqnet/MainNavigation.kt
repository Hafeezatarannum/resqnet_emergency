...
import com.example.resqnet.ui.sos.FindVolunteersRadarScreen
...
fun MainNavigation() {
    ...
    val showBars = currentRoute !in listOf(
        ...
        "role_selection", "location_permission", "notification_permission", "active_sos", "find_volunteers"
    )
                        ...
                        VolunteerDrawerContent(
                            ...
                            onNavigate = { route ->
                                ...
                                navController.navigate(route) {
                                    popUpTo("volunteer_dashboard") { saveState = true }
                ...
                bottomBar = {
                    if (showBars) {
                        ResQNetBottomBar(
                            ...
                            onNavigate = { route ->
                                navController.navigate(route) {
                                    val popTarget = if (isVolunteer) "volunteer_dashboard" else "home"
                                    popUpTo(popTarget) { saveState = true }
                                    launchSingleTop = true
                                    restoreState = true
                                }
                            }
                    ...
                    NavHost(navController = navController, startDestination = "splash") {
                        ...
                        composable("welcome") {
                            WelcomeScreen(
                                ...
                                onNavigateToSos = { navController.navigate("active_sos") }
                            )
                        }
                        ...
                        composable("home") {
                            HomeScreen(
                                onTriggerSos = { navController.navigate("active_sos") },
                                onNavigateToVoiceSos = { navController.navigate("active_sos") },
                                onNavigateToShakeSos = { navController.navigate("active_sos") },
                                ...
                                onNavigateToAlerts = { navController.navigate("find_volunteers") },
                                onNavigateToTracking = { sosId -> navController.navigate("live_tracking") }
                            )
                        }

                        composable("find_volunteers") {
                            FindVolunteersRadarScreen(
                                onCancel = { navController.popBackStack() },
                                onVolunteerMatched = { volId -> navController.navigate("live_tracking") }
                            )
                        }
