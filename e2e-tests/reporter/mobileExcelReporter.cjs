...
class MobileExcelReporter extends mocha.reporters.Base {
  constructor(runner, options) {
    ...
    runner.on('pass', (test) => {
      this.results.push({
        suite: test.parent ? test.parent.title : 'General Mobile Module',
        title: test.title,
        state: 'Passed',
    ...
    runner.on('fail', (test, err) => {
      this.results.push({
        ...
        state: 'Failed',
        duration: test.duration || 0,
        error: err ? (err.message || String(err)) : 'Unknown mobile failure'
      });
    });

    runner.on('end', async () => {
      console.log('\n========================================');
      console.log('Generating Mobile Appium E2E Excel Report...');
  ...
  async generateReport() {
    ...
    workbook.creator = 'ResQNet Appium Mobile Automation Suite';
    ...
    const summarySheet = workbook.addWorksheet('Mobile Executive Summary');
    summarySheet.views = [{ showGridLines: true }];

    summarySheet.mergeCells('A1:E1');
    const titleCell = summarySheet.getCell('A1');
    titleCell.value = 'ResQNet Android Mobile - Appium E2E Test Execution Report';
    titleCell.font = { name: 'Calibri', size: 16, bold: true, color: { argb: 'FFFFFFFF' } };
    titleCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF203764' } };
    titleCell.alignment = { horizontal: 'center', vertical: 'middle' };
    summarySheet.getRow(1).height = 35;

    summarySheet.mergeCells('A2:E2');
    const subTitle = summarySheet.getCell('A2');
    subTitle.value = `Execution Date: ${new Date().toLocaleString()} | Framework: Appium / WebdriverIO / UiAutomator2`;
    ...
    const metricHeaderRow = summarySheet.addRow(['Mobile Parameter', 'Value', '', 'Status Summary', 'Count']);
    ...
    const metrics = [
      ['Target Mobile App ID', 'com.resqnet.app (Android)', '', 'Total Mobile Test Cases', totalTests],
      ['Automation Engine', 'Appium / UiAutomator2', '', 'Passed Test Cases', passedTests],
      ['Total Execution Duration', `${(totalTimeMs / 1000).toFixed(2)} seconds`, '', 'Failed Test Cases', failedTests],
    ...
    // --- SHEET 2: DETAILED MOBILE TEST RESULTS ---
    const detailsSheet = workbook.addWorksheet('Detailed Mobile Test Cases');
    detailsSheet.views = [{ showGridLines: true }];

    detailsSheet.columns = [
      { header: '#', key: 'id', width: 8 },
      { header: 'Mobile Module / Screen', key: 'suite', width: 35 },
    ...
    const reportDir = process.env.REPORT_DIR
      ...
      : path.join(__dirname, '..', 'reports', 'mobile');
    ...
    const reportPath = path.join(reportDir, `E2E_Mobile_Test_Report_ResQNet_${timestamp}.xlsx`);

    await workbook.xlsx.writeFile(reportPath);
    console.log(`Mobile Appium E2E Excel Report successfully saved at:\n--> ${reportPath}`);
  }
}

module.exports = MobileExcelReporter;
